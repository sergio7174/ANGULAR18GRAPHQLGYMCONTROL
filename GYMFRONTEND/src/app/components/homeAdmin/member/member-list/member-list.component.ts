...
private readonly toast =  inject(ToastrService);
  ...
  // to see the images on list you must change the baseUrl between environment.endpointI,
  // and environment.endpoint cause sometimes I change the imageUrl depending on witch backend Im using
...
getDatamembers() {
  ...
  this.memberService.getmemberList().pipe(
   ...
   if (this.Datamembers.Members.length > 0 ){
     // For loop to get all members in database
     let index = 0;

   while (index <= this.Datamembers.Members.length) {
      //alert("this.Datamembers.Members.length: "+this.Datamembers.Members.length);
      //alert(" - Iam at member-list component - line 101 - index : " + index);
      ...
      //alert ("Line-111 - this.daysLeft_mathFloor[index]: "+ this.daysLeft_mathFloor[index] + '' + 'index: ' + index);
      ...
      if (index === this.Datamembers.Members.length){
